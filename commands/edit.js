module.exports = {
    ...
    async execute(msg, args, client, con2) {
        ...
        if (args.length < 3 || args.length > 5 || args[1].length > 1) {
            return msg.channel.send('command syntax failure!! <:bonk:720758421514878998>\n\nCommand usage:\n`=edit [submission link] [S/M/L] [quality 1, 1.5, 2] [incompletion 0.5, 1] [collaborators]`')
        }
        ...
        try {
            ...
            //check if submission is already submitted. if not, then cancel
            ...
            const myQuery = `update submissions set msg_id = ${msgId}, points_total = ${pointsTotal}, building_type = '${buildingType}', quality = ${quality}, incompletion = ${incompletion}, collaboration = ${collaborators}, user_id = ${userId} where msg_id = ${msgId}`
            
            //write to db
            con2.query(myQuery, (err, result) => {
                if (err) throw err
                msg.channel.send(
                    `SUBMISSIONS SUCCESSFULLY UPDATED YAY!!!<:HAOYEEEEEEEEEEAH:908834717913186414>\n\n<@${userId}> has gained **${pointsTotal} points!!!**\n\n*__Points breakdown:__*\nBuilding type: ${buildingType}\nQuality multiplier: ${quality}\nINCOMPLETION multiplier: ${incompletion}\nCollaborators: ${collaborators}`
                )
                submissionMsg.react('✅')
            })
        } catch (err) {
